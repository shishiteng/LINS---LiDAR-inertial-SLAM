...
void MapBuilder::Transform4DAssociateToMap()
{
  ...
  // lio的位姿加上补偿了yaw的旋转得出新的预测位姿，(transform_sum里是lio最新的pose，包含了rp)
  // lio位姿跟lio-mapping旋转差异可能偏大，所以lio-mapping的预测位姿要补偿yaw
...
void MapBuilder::PublishMapBuilderResults()
{
  ...
  path_aft_mapped_.header.stamp = time_laser_odometry_;
...
void MapBuilder::ProcessMap()
{
  ...
  if (enable_4d_)
  {
    // 用lio位姿预测lio-mapping的位姿，只用yaw的变化预测
  ...
  //std::cout << "center_before: " << center_cube_i << " " << center_cube_j << " " << center_cube_k << std::endl;
  {
  ...
  // std::cout << "center_after: " << center_cube_i << " " << center_cube_j << " " << center_cube_k << std::endl;
  // std::cout << "laser_cloud_cen: " << laser_cloud_cen_length_ << " " << laser_cloud_cen_width_ << " " << laser_cloud_cen_height_ << std::endl;
...
void MapBuilder::OptimizeMap()
{
  ...
  // 线特征的残差：w(Rx+p)+d,w代表点到线的垂直向量，d代表点到线的距离，整体上代表点x(向量)在点线垂直向量w上的的投影与d的残差最小
  // 面特征的残差：w(Rx+p)+d,w代表平面系数，d代表点面距离
  ...
  for (size_t iter_count = 0; iter_count < num_max_iterations_; ++iter_count)
  {
    ...
    for (int i = 0; i < laser_cloud_corner_stack_size; ++i)
    {
      ...
      if (point_search_sq_dis[4] < min_match_sq_dis_)
      {
        ...
        if (mat_D1(0, 2) > 3 * mat_D1(0, 1))
        {
          ...
          // a*b*sin(theta)/2c计算点到底边的距离（三角形面积法）
          Eigen::Vector3f a012_vec = (X0 - X1).cross(X0 - X2);

          // 点到底边的垂直向量(这里只有方向，单位向量)
          Eigen::Vector3f normal_to_point = ((X1 - X2).cross(a012_vec)).normalized();

          float a012 = a012_vec.norm(); // 2倍的三角形面积
          float l12 = (X1 - X2).norm(); // 底边大小
    ...
    {
      ROS_WARN("not enough matched points for optimization: %lu_%lu", iter_count, laser_cloud_sel_size);
      continue;
    }

    // 线性化后，解线性方程组Ax=B
    // 这里求的R是delta z，再加上其它两个轴的微小扰动，所以是3自由度，计算出来后可以在sophus::SO3上直接累加
    ...
    for (int i = 0; i < laser_cloud_sel_size; i++)
    {
      ...
      Eigen::Vector3f J_r =
          -w.transpose() * (transform_tobe_mapped_.rot * SkewSymmetric(p)) * transform_tobe_mapped_.rot.inverse().toRotationMatrix() * right_info_mat;
    ...
    // std::cout << "mat_X.head<3>() aft: " << mat_X.head<3>().transpose() << std::endl;

    // 关于退化的原理，见ZhangJi论文:On Degeneracy of Optimization-based State Estimation Problems
    if (iter_count == 0)
    {
    ...
    if (iter_count == num_max_iterations_ - 1)
      ROS_DEBUG("max iterations reached: %lu, delta(%lfrad, %lfm)", iter_count, delta_r, delta_t);
  }
